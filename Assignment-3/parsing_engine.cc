...
namespace semantic_analysis {
    std::string traverseTreeSemantically(Node* node, symbol_table::SymbolTable* sym_table, bool& encounteredError, bool printError = true) {
        if(node->type == "Program") {
            if(!node->children.empty()) {
                ...
                for(auto i = node->children.begin(); i != node->children.end(); ++i) {
                    ...
                    traverseTreeSemantically(*i, sym_table, encounteredError);
                    sym_table->exitScope();
                }
            }
        }
        ...
        if(node->type == "Var declarations") {
            if(!node->children.empty()) {
                for(auto i = node->children.begin(); i != node->children.end(); i++) {
                    std::string varDec = traverseTreeSemantically((*i), sym_table, encounteredError);
                    if(varDec != "int" && varDec != "int[]" && varDec != "boolean") {
                        auto var_type = (symbol_table::Class*)sym_table->getRootScope()->lookup(varDec);
 
                        if (var_type == NULL) {
                            if (printError) std::cerr << "[14.1] @error at line " + std::to_string((*i)->lineno) + ": Instance of '" + varDec + "' is not defined" << std::endl;
        ...
        if(node->type == "Return") {
            std::string returnType = traverseTreeSemantically(node->children.back(), sym_table, encounteredError);
            std::string methodType = ((symbol_table::Method*)sym_table->getCurrentScope()->getScopeContext())->getType();
            // Add logic later
            if (returnType != methodType) {
                if (printError) std::cerr << "[0.1] @error at line " + std::to_string(node->children.back()->lineno) + ": cannot convert return type of '" + returnType + "' to '" + methodType + "'" << std::endl;
                encounteredError = true;
            }
        }
        ...
        if(node->type == "Statement") {
            traverseTreeSemantically(node->children.front(), sym_table, encounteredError);
        }
        ...
        if(node->type == "If-Expression-Statement") {
            ...
            std::string lhsType = traverseTreeSemantically(node->children.front(), sym_table, encounteredError, false);
            // std::cout << "THE TYPE IS: " << lhsType << std::endl;
            if (lhsType != "boolean") {
                if (printError) std::cerr << "[0] @error at line " + std::to_string(node->lineno) + ": cannot convert '" + lhsType + "' to 'boolean'" << std::endl;
                encounteredError = true;
            }
        }
        if(node->type == "If/Else-Expression-Statement") {
            ...
            // std::cout << "THE TYPE IS: " << lhsType << std::endl;
            if (lhsType != "boolean") {
                if (printError) std::cerr << "[1] @error at line " + std::to_string(node->lineno) + ": cannot convert '" + lhsType + "' to 'boolean'" << std::endl;
                encounteredError = true;
            }
        }
        if(node->type == "While-Statement") {
            ...
            if (lhsType != "boolean") {
                if (printError) std::cerr << "[2] @error at line " + std::to_string(node->lineno) + ": cannot convert '" + lhsType + "' to 'boolean'" << std::endl;
                encounteredError = true;
            }
        }
        if(node->type == "SysPrintLn") {
            std::string lhsType = traverseTreeSemantically(node->children.front(), sym_table, encounteredError, &printError);

            if (lhsType != "int" && lhsType != "boolean") {
                if (printError) std::cerr << "[3] @error at line " + std::to_string(node->lineno) + ": cannot convert '" + lhsType + "' to 'int'" << std::endl;
                encounteredError = true;
            }
        }
        if(node->type == "Identifier assign") {
            std::string lhsType = traverseTreeSemantically(node->children.front(), sym_table, encounteredError, &printError);
            std::string rhsType = traverseTreeSemantically(node->children.back(), sym_table, encounteredError, &printError);

            if (lhsType != rhsType) {
                if (printError && lhsType != "undefined" && rhsType != "undefined") std::cerr << "[4] @error at line " + std::to_string(node->lineno) + ": cannot convert '" + lhsType + "' to '" + rhsType + "' in assignment" << std::endl;
                encounteredError = true;
            }
        }
        if(node->type == "ArrayPositionAssignOp") {
            std::string lhsType = traverseTreeSemantically(node->children.front(), sym_table, encounteredError, printError);
            std::string posType = traverseTreeSemantically(*(++node->children.begin()), sym_table, encounteredError, printError);
            std::string rhsType = traverseTreeSemantically(node->children.back(), sym_table, encounteredError, printError);
            // Check assignment position
            if(lhsType != "int[]") {
                if (printError) std::cerr << "[5] @error at line " + std::to_string(node->lineno) + ": Expression must be of type int[]" << std::endl;
                encounteredError = true;
            }
            if(posType != "int") {
                if (printError) std::cerr << "[6] @error at line " + std::to_string(node->lineno) + ": Cannot convert assignment position '" + posType + "' to int" << std::endl;
                encounteredError = true;
            }
            if(rhsType != "int") {
                if (printError) std::cerr << "[7] @error at line " + std::to_string(node->lineno) + ": Cannot convert assignment value '" + rhsType + "' to int" << std::endl;
                encounteredError = true;
            }
        }
        ...
        if(node->type == "LogicalAndExpression" || node->type == "LogicalOrExpression") {
            ...
            if(lhsType != "boolean" || rhsType != "boolean") {
                if (printError) std::cerr << "[8] @error at line " + std::to_string(node->lineno) + ": " + node->type + " requires both operands to be of type 'boolean', but found '" + lhsType + "' and '" + rhsType + "'" << std::endl;
                encounteredError = true;
            }
        ...
        if(node->type == "LesserThanExpression" || node->type == "GreaterThanExpression") {
            ...
            if(lhsType != "int" || rhsType != "int") {
                if (printError) std::cerr << "[9] @error at line " + std::to_string(node->lineno) + ": " + node->type + " requires both operands to be of type 'int', but found '" + lhsType + "' and '" + rhsType + "'" << std::endl;
                encounteredError = true;
            }
        ...
        if(node->type == "EqualExpression") {
            ...
            if(lhsType != rhsType) {
                if (printError) std::cerr << "[10] @error at line " + std::to_string(node->lineno) + ": Equal expression requires both operands to be of the same type, but found '" + lhsType + "' and '" + rhsType + "'" << std::endl;
                encounteredError = true;
            }
        ...
        node->type == "MultExpression" || node->type == "DivExpression") {
            ...
            if(lhsType != "int" || rhsType != "int") {
                if (printError) std::cerr << "[11] @error at line " + std::to_string(node->lineno) + ": " + node->type + " requires both operands to be of type 'int', but found '" + lhsType + "' and '" + rhsType + "'" << std::endl;
                encounteredError = true;
            }
        ...
        if (node->type == "Array access") {
            ...
            if(lhsType != "int[]") {
                if (printError) std::cerr << "[12.1] @error at line " + std::to_string(node->lineno) + ": accessing array requires an array of type 'int[]', but found '" + lhsType + "'" << std::endl;
                encounteredError = true;
            }
            else if(rhsType != "int") {
                if (printError) std::cerr << "[12.2] @error at line " + std::to_string(node->lineno) + ": accessing array requires access position of type 'int' but found '" + rhsType + "'" << std::endl;
                encounteredError = true;
            }
        ...
        if (node->type == "Expression" && node->value == "length") {
            std::string lhsType = traverseTreeSemantically(node->children.front(), sym_table, encounteredError);

            if(lhsType != "int[]") {
                if (printError) std::cerr << "[13] @error at line " + std::to_string(node->lineno) + ": length of expression requires expression of type 'int[]', but found '" + lhsType + "'" << std::endl;
                encounteredError = true;
            }
        ...
        if(node->type == "ExpressionCallMethod") {
            ...
            // First step
            std::string lhsName = traverseTreeSemantically(node->children.front(), sym_table, encounteredError);
            ...
            if (lhs_class == NULL) {
                if (printError) std::cerr << "[14.1] @error at line " + std::to_string(node->lineno) + ": Instance of '" + lhsName + "' is not defined" << std::endl;
                encounteredError = true;
                return "undefined";
            }
            ...
            if(_method == NULL) {
                if (printError) std::cerr << "[14.2] @error at line " + std::to_string(node->lineno) + ": Method '"+ mhsName + "' of instance '" + lhsName + "' is not defined" << std::endl;
                encounteredError = true;
                return "undefined";
            }
            ...
            if (it != node->children.end()) {
                ...
                if (n_input_params != _method->getParamSize()) {
                    if (printError) std::cerr << "[15] @error at line " + std::to_string(node->lineno) + ": Method '"+ mhsName + "' has (" + std::to_string(_method->getParamSize()) + ") parameter(s), (" + std::to_string(n_input_params) + ") was given" << std::endl;
                    encounteredError = true;
                }
                ...
                for (auto i = (*it)->children.begin(); i != (*it)->children.end(); i++) {
                    ...
                    std::string lhsName = traverseTreeSemantically(*i, sym_table, encounteredError);
                    if (lhsName != _method_params[j]->getType()) {
                        if (printError) std::cerr << "[16] @error at line " + std::to_string(node->lineno) + ": expected parameter type (" + _method_params[j]->getType() + ") but (" + lhsName + ") was given" << std::endl;
                        encounteredError = true;
                    }
                    j++;
                }
                ...
                if(_method->getParamSize() > 0) {
                    if (printError) std::cerr << "[15] @error at line " + std::to_string(node->lineno) + ": Method '"+ mhsName + "' has (" + std::to_string(_method->getParamSize()) + ") parameter(s), (0) was given" << std::endl;
                    encounteredError = true;
                }
            }
        ...
        if(node->type == "Identifier" || node->type == "this") {
            ...
            if (identifier == NULL) {
                if (printError) std::cerr << "[17] @error at line " + std::to_string(node->lineno) + ": Identifier '" + node->value + "' is not defined" << std::endl;
                printError = false;
                encounteredError = true;
                return "undefined";
            ...
            if(identifier->getLineno() != -1) {
                ...
                if (used_line < declared_line) {
                    if (printError) std::cerr << "[17] @error at line " + std::to_string(node->lineno) + ": Identifier '" + node->value + "' is not defined yet" << std::endl;
                    printError = false;
                    encounteredError = true;
                    return "undefined";
                }
        ...
        if(node->type == "NewArray" && node->value == "int") {
            ...
            if(lhsType != "int") {
                if (printError) std::cerr << "[19] @error at line " + std::to_string(node->lineno) + ": length of expression requires expression of type 'int[]', but found '" + lhsType + "'" << std::endl;
                encounteredError = true;
            }
        ...
        if(node->type == "NewIstance") {
            ...
            if (instance == NULL) {
                if (printError) std::cerr << "[20] @error at line " + std::to_string(node->lineno) + ": Instance of '" + node->value + "' is not defined" << std::endl;
                encounteredError = true;
                return "undefined";
            }
        ...
        if(node->type == "NotOperation") {
            std::string type = traverseTreeSemantically(node->children.front(), sym_table, encounteredError, false);

            if(type != "boolean") {
                if (printError) std::cerr << "[21] @error at line " + std::to_string(node->lineno) + ": no known conversion for '" + type + "' to 'bool'" << std::endl;
                encounteredError = true;
            }
    ...
    errCodes semantic_analysis(Node* node, symbol_table::SymbolTable* sym_table) {
        try {
            ...
            if(errorsEncountered) throw std::runtime_error("");
