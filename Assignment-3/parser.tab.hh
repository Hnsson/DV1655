...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      // root
      // goal
      // classDeclaration_inf
      // mainClass
      // classDeclaration
      // varDeclaration_inf
      // varDeclaration
      // methodDeclaration_inf
      // methodDeclaration
      // methodHeader
      // methodParam
      // methodContent
    ...
    VOID = 282,                    // VOID
    MAIN = 283,                    // MAIN
    STRINGDEC = 284,               // STRINGDEC
    INTDEC = 285,                  // INTDEC
    BOOL = 286,                    // BOOL
    IF = 287,                      // IF
    ELSE = 288,                    // ELSE
    WHILE = 289,                   // WHILE
    SYSPRINTLN = 290,              // SYSPRINTLN
    LENGTH = 291,                  // LENGTH
    TRUE = 292,                    // TRUE
    FALSE = 293,                   // FALSE
    THIS = 294,                    // THIS
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_VOID = 27,                             // VOID
        S_MAIN = 28,                             // MAIN
        S_STRINGDEC = 29,                        // STRINGDEC
        S_INTDEC = 30,                           // INTDEC
        S_BOOL = 31,                             // BOOL
        S_IF = 32,                               // IF
        S_ELSE = 33,                             // ELSE
        S_WHILE = 34,                            // WHILE
        S_SYSPRINTLN = 35,                       // SYSPRINTLN
        S_LENGTH = 36,                           // LENGTH
        S_TRUE = 37,                             // TRUE
        S_FALSE = 38,                            // FALSE
        S_THIS = 39,                             // THIS
        ...
        S_root = 44,                             // root
        S_goal = 45,                             // goal
        S_classDeclaration_inf = 46,             // classDeclaration_inf
        S_mainClass = 47,                        // mainClass
        S_classDeclaration = 48,                 // classDeclaration
        S_varDeclaration_inf = 49,               // varDeclaration_inf
        S_varDeclaration = 50,                   // varDeclaration
        S_methodDeclaration_inf = 51,            // methodDeclaration_inf
        S_methodDeclaration = 52,                // methodDeclaration
        S_methodHeader = 53,                     // methodHeader
        S_methodParam = 54,                      // methodParam
        S_methodContent = 55,                    // methodContent
        S_methodContentParam = 56,               // methodContentParam
        S_methodContentItem = 57,                // methodContentItem
        S_type = 58,                             // type
        S_statement_inf = 59,                    // statement_inf
        S_statement = 60,                        // statement
        S_expression_inf = 61,                   // expression_inf
...
switch (yykind)
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_LTOP (std::string v)
      {
        return symbol_type (token::LTOP, std::move (v));
      }
#else
      static
      symbol_type
      make_LTOP (const std::string& v)
      {
        return symbol_type (token::LTOP, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_GTOP (std::string v)
      {
        return symbol_type (token::GTOP, std::move (v));
      }
#else
      static
      symbol_type
      make_GTOP (const std::string& v)
      {
        return symbol_type (token::GTOP, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_STRINGDEC (std::string v)
      {
        return symbol_type (token::STRINGDEC, std::move (v));
      }
#else
      static
      symbol_type
      make_STRINGDEC (const std::string& v)
      {
        return symbol_type (token::STRINGDEC, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_INTDEC (std::string v)
      {
        return symbol_type (token::INTDEC, std::move (v));
      }
#else
      static
      symbol_type
      make_INTDEC (const std::string& v)
      {
        return symbol_type (token::INTDEC, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_SYSPRINTLN (std::string v)
      {
        return symbol_type (token::SYSPRINTLN, std::move (v));
      }
#else
      static
      symbol_type
      make_SYSPRINTLN (const std::string& v)
      {
        return symbol_type (token::SYSPRINTLN, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
